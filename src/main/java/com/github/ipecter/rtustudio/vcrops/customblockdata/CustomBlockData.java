...
package com.github.ipecter.rtustudio.vcrops.customblockdata;

import com.github.ipecter.rtustudio.vcrops.customblockdata.events.CustomBlockDataRemoveEvent;
...
public class CustomBlockData implements PersistentDataContainer {
     ...
     */
    private static final NamespacedKey PERSISTENCE_KEY = Objects.requireNonNull(NamespacedKey.fromString("com.github.ipecter.rtustudio.vcrops.customblockdata:protected"), "Could not create persistence NamespacedKey");
     ...
     * @param plugin Plugin
     */
    public CustomBlockData(final @NotNull Block block, final @NotNull RSPlugin plugin) {
    ...
    public CustomBlockData(final @NotNull World world, final int x, final int y, final int z, final @NotNull RSPlugin plugin) {
    ...
    static void setDirty(Plugin plugin, Map.Entry<UUID, BlockVector> blockEntry) {
        ...
        if (MinecraftVersion.isFolia()) {
            CraftScheduler.runLater(plugin, () -> DIRTY_BLOCKS.remove(blockEntry), 1L);
        } else {
            CraftScheduler.run(plugin, () -> DIRTY_BLOCKS.remove(blockEntry));
        }
    }
    ...
    public void copyTo(PersistentDataContainer other, boolean replace) {
        ...
        CustomBlockData target = (CustomBlockData) other;
        getKeys().forEach(key -> {
            PersistentDataType dataType = getDataType(this, key);
            if (dataType == null) return;
            if (replace) {
                target.set(key, dataType, get(key, dataType));
            } else {
                if (!target.has(key)) target.set(key, dataType, get(key, dataType));
            }
        });
    }
